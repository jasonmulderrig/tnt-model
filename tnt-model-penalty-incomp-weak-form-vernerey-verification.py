...
t_u_applied_A = t_initial + t_applied
t_u_rest_A = t_u_applied_A + t_rest
t_u_applied_B = t_u_rest_A + t_applied

dt = 0.0025

num_tsteps_u_applied_A = int((t_u_applied_A-t_initial)/dt)
num_tsteps_u_rest_A = int((t_u_rest_A-t_u_applied_A)/dt)
num_tsteps_u_applied_B = int((t_u_applied_B-t_u_rest_A)/dt)
num_tsteps_u_rest_B = int((t_final-t_u_applied_B)/dt)+1

num_tsteps_total = num_tsteps_u_applied_A + num_tsteps_u_rest_A + num_tsteps_u_applied_B + num_tsteps_u_rest_B
time = np.linspace(t_initial, t_final, num_tsteps_total, endpoint=True)

u_initial = 0
u_final = 1

u_12_applied_A = np.linspace(u_initial, u_final, num_tsteps_u_applied_A, endpoint=False)
u_12_rest_A = u_final*np.ones(num_tsteps_u_rest_A)
u_12_applied_B = u_final*np.ones(num_tsteps_u_applied_B)
u_12_rest_B = u_final*np.ones(num_tsteps_u_rest_B)
u_12_list = np.concatenate((u_12_applied_A, u_12_rest_A, u_12_applied_B, u_12_rest_B))

u_22_applied_A = u_initial*np.ones(num_tsteps_u_applied_A)
u_22_rest_A = u_initial*np.ones(num_tsteps_u_rest_A)
u_22_applied_B = np.linspace(u_initial, u_final, num_tsteps_u_applied_B, endpoint=False)
u_22_rest_B = u_final*np.ones(num_tsteps_u_rest_B)
u_22_list = np.concatenate((u_22_applied_A, u_22_rest_A, u_22_applied_B, u_22_rest_B))
...
modelname = "tnt-model-penalty-incomp-weak-form-vernerey-verification"
...
sigma_G_12_center_point_array = np.zeros((num_tsteps_total,1))
F_12_center_point_array       = np.zeros((num_tsteps_total,1))
L_prior_12_center_point_array = np.zeros((num_tsteps_total,1))
mu_12_center_point_array      = np.zeros((num_tsteps_total,1))

sigma_G_22_center_point_array = np.zeros((num_tsteps_total,1))
F_22_center_point_array       = np.zeros((num_tsteps_total,1))
L_prior_22_center_point_array = np.zeros((num_tsteps_total,1))
mu_22_center_point_array      = np.zeros((num_tsteps_total,1))

Psi_center_point_array = np.zeros((num_tsteps_total,1))
lmbda_c_center_point_array = np.zeros((num_tsteps_total,1))
...
deformation_12 = Expression("u_xy", u_xy=0.0, degree=0)
deformation_22 = Expression("u_yy", u_yy=0.0, degree=0)
...
bc_I = DirichletBC(V_u.sub(0), Constant(0.0), bottom_boundary)
bc_II = DirichletBC(V_u.sub(1), Constant(0.0), bottom_boundary)
bc_III = DirichletBC(V_u.sub(0), deformation_12, top_boundary)
bc_IV = DirichletBC(V_u.sub(1), deformation_22, top_boundary)

bcs = [bc_I, bc_II, bc_III, bc_IV]
...
for t_ind, t in enumerate(time):
    ...
    u_12_def = u_12_list[t_ind]
    u_22_def = u_22_list[t_ind]

    deformation_12.u_xy = u_12_def
    deformation_22.u_yy = u_22_def
    ...
    sigma_G_12_center_point_array[t_ind] = sigma_G_val(center_point)[1]
    F_12_center_point_array[t_ind]       = F_val(center_point)[1]
    L_prior_12_center_point_array[t_ind] = L_prior_val(center_point)[1]
    mu_12_center_point_array[t_ind]      = mu_val(center_point)[1]
    ...
    lmbda_c_center_point_array[t_ind] = lmbda_c_val(center_point)
...
plt.figure(0)
plt0data1, = plt.plot(time, u_12_list, linestyle='-', color='red', alpha=1, linewidth=2.5, label=r'$u_{12}$')
plt0data2, = plt.plot(time, u_22_list, linestyle='-', color='blue', alpha=1, linewidth=2.5, label=r'$u_{22}$')
plt.legend(handles=[plt0data1, plt0data2], loc='upper right', fontsize=14, frameon=True, fancybox=True, framealpha=1, shadow=True, borderpad=1)
plt.xlabel(r'$t$', fontsize=30)
plt.ylabel(r'$u$', fontsize=30)
plt.tight_layout()
plt.savefig(savedir+"u_vs_t.pdf", transparent=True)
# plt.savefig(savedir+"u_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(1)
plt1data1, = plt.plot(time, sigma_G_12_center_point_array, linestyle='-', color='red', label=r'$\frac{\sigma_{12}}{G}$', alpha=1, linewidth=2.5)
...
# plt.savefig(savedir+"sigma_G_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(2)
plt2data1, = plt.plot(time, F_12_center_point_array, linestyle='-', color='red', label=r'$F_{12}$', alpha=1, linewidth=2.5)
...
# plt.savefig(savedir+"F_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(3)
plt3data1, = plt.plot(time, L_prior_12_center_point_array, linestyle='-', color='red', label=r'$L_{12}$', alpha=1, linewidth=2.5)
...
# plt.savefig(savedir+"L_prior_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(4)
plt4data1, = plt.plot(time, mu_12_center_point_array, linestyle='-', color='red', label=r'$\mu_{12}$', alpha=1, linewidth=2.5)
...
# plt.savefig(savedir+"mu_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(5)
plt.plot(time, 2*Psi_center_point_array, linestyle='-', color='black', alpha=1, linewidth=2.5)
plt.xlabel(r'$t$', fontsize=30)
plt.ylabel(r'$\frac{2\Psi}{G}$', fontsize=30)
plt.tight_layout()
plt.savefig(savedir+"Psi_vs_t.pdf", transparent=True)
# plt.savefig(savedir+"Psi_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()

plt.figure(6)
plt.plot(time, lmbda_c_center_point_array, linestyle='-', color='black', alpha=1, linewidth=2.5)
plt.xlabel(r'$t$', fontsize=30)
plt.ylabel(r'$\lambda_c$', fontsize=30)
plt.tight_layout()
plt.savefig(savedir+"lambda_c_vs_t.pdf", transparent=True)
# plt.savefig(savedir+"lambda_c_vs_t.eps", format='eps', dpi=1000, transparent=True)
plt.close()
